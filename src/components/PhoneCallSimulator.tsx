...
const PRESET_PROMPTS = [
  {
    label: "☕ 1. สอบถามเมล็ดกาแฟ",
    text: "สวัสดีค่ะ ที่ร้านมีเมล็ดกาแฟคั่วแบบไหนแนะนำบ้างคะ? กำลังจะเปิดร้านใหม่ค่ะ"
  },
  {
    label: "🛠️ 2. แจ้งซ่อมเครื่องชงน้ำไม่ไหล",
    text: "สวัสดีค่ะ พอดีเครื่องชงกาแฟที่ร้านน้ำไม่ไหลเลยค่ะ รบกวนส่งเรื่องให้ช่างให้หน่อยได้ไหมคะ"
  },
  {
    label: "📞 3. แจ้งซ่อมเครื่องชงไม่ร้อน",
    text: "เครื่องชงกาแฟน้ำไม่ร้อนเลยค่ะ ใช้งานไม่ได้เลย อยากแจ้งเรื่องเคลมหรือซ่อมด่วนค่ะ"
  },
  {
    label: "📈 4. ขอเรตราคาส่งยกลัง",
    text: "สนใจสั่งซื้อเมล็ดกาแฟคั่วกลางยกลังปริมาณมากค่ะ ขอตารางราคาส่งยกลังและใบเสนอราคาด่วนเลยค่ะ"
  },
  {
    label: "👤 5. ขอคุยกับเจ้าหน้าที่พนักงาน",
    text: "ฮัลโหล ขอโอนสายคุยกับคุณขวัญหรือพนักงานโดยตรงหน่อยค่ะ ไม่อยากคุยกับบอทแล้วค่ะ"
  }
];
...
const simulateLocalResponse = (inputText: string, gender: "female" | "male") => {
  ...
  // Pre-configured intelligent conversational filler variations
  const helloPhrases = [
    "สวัสดีค่ะ! น้องธันวาจาก December Day Coffee ยินดีต้อนรับนะคะ วันนี้สนใจเมล็ดกาแฟอร่อย ๆ หรือมีเรื่องเครื่องชงขัดข้องให้ช่วยเหลือ แจ้งเรื่องได้เลยค่ะ",
    "ฮัลโหล ยินดีต้อนรับค่ะ! น้องธันวามาแล้วค่ะ วันนี้สนใจเมล็ดกาแฟคั่วเรตราคาส่ง หรือแจ้งปัญหาแจ้งได้เลยนะคะ เดี๋ยวหนูดูแลให้ค่ะ"
  ];

  const wantsStaffPhrases = [
    "รับทราบและเข้าใจเลยค่ะ เดี๋ยวน้องธันวาขอกดโอนสายส่งต่อให้กับคุณขวัญที่เบอร์ 096-163-1456 เพื่อดูแลให้คุณลูกค้าด่วนเลยนะคะ รอสักครู่เดียวค่ะ",
    "อ๋อ ได้เลยค่ะ! เดี๋ยวหนูรีบกดโอนสายด่วนตรงไปยังพี่ขวัญฝ่ายบริการลูกค้าดูแลให้ทันทีเลยค่ะ กรุณารอสักครู่เดียวค๊า"
  ];

  const isIssuePhonePhrases = [
    "ได้รับเบอร์ติดต่อและรับเรื่องประสานงานช่างเรียบร้อยค่ะ เดี๋ยวน้องธันวาขอส่งเรื่องโอนสายด่วนให้ทีมช่างเทคนิคติดต่อกลับทันทีเลยนะคะ รอสายสักครู่ค่ะ",
    "เข้าใจเลยค่ะ เดี๋ยวหนูรีบโอนสายส่งต่อฝ่ายช่างเทคนิคให้ติดต่อกลับด่วนที่สุดเพื่อเข้าไปดูแลหน้าร้านให้เลยนะคะ รอสักครู่เดียวค่ะ"
  ];

  const isIssueNoPhonePhrases = [
    "อุ๊ย ต้องขอประทานโทษด้วยนะคะที่เกิดปัญหาเครื่องชงขัดข้องหน้าร้าน ทางเรามีบริการช่างซ่อมเครื่องชงครบวงจร ล้างตะกรัน และเปลี่ยนอะไหล่หัวชงค่ะ รบกวนแอดไลน์ช่างด่วนที่ @decservice หรือจะทิ้งเบอร์โทรติดต่อกลับไว้ตรงนี้ เดี๋ยวน้องธันวาประสานงานช่างให้ทันทีเลยค่ะ",
    "เอ๊ยย ต้องขอโทษด้วยจริง ๆ นะคะ ทางเรามีทีมช่างดูแลเครื่องชง เครื่องบด เครื่องปั่นครบวงจรเลยค่ะ แนะนำแอดไลน์ช่างเทคนิคด่วนที่ @decservice (https://lin.ee/WXYf27n) หรือพิมพ์ทิ้งชื่อร้านและเบอร์ติดต่อไว้ เดี๋ยวหนูประสานช่างให้ติดต่อกลับด่วนที่สุดเลยค่ะ"
  ];

  const hasPhoneCoffeePhrases = [
    "ได้รับเบอร์ติดต่อเรียบร้อยค่ะ เดี๋ยวน้องธันวารีบส่งเรื่องต่อให้ฝ่ายขายโทรกลับไปจัดทำใบเสนอราคาเรตราคาส่งพิเศษให้ทันทีเลยนะคะ รอสักครู่ค่ะ",
    "โอเคค่ะ ได้รับเบอร์โทรเรียบร้อย เดี๋ยวหนูประสานงานฝ่ายขายโทรกลับเพื่อเสนอราคาพิเศษยกลังให้ด่วนเลยค่ะ รอรับสายสักครู่ค๊า"
  ];

  const dynamicIntroPhrases = [
    "ยินดีเลยค่ะ! สำหรับรายละเอียดของตัว ",
    "อ๋อ เข้าใจเลยค่ะ! สำหรับข้อมูลของรุ่น ",
    "ตัวนี้ขายดีและยอดนิยมมากเลยค่ะ! สำหรับข้อมูลของ ",
    "รับทราบเลยค่ะ! ถ้าเป็นรายละเอียดของ "
  ];

  const defaultCoffeePhrases = [
    "เรามีเมล็ดกาแฟราคาส่งยอดนิยม เช่น S5 Premium Dark สำหรับกาแฟนมรสเข้มข้น และ Colombia Peach Candy หอมหวานพีชฟุ้งๆ ค่ะ รบกวนแอดไลน์ขอตารางราคายกลังที่ Line OA: @decemberdaycoffee หรือแจ้งเบอร์โทรศัพท์ทิ้งไว้ได้เลยนะคะ",
    "มีเมล็ดกาแฟราคาส่งเกรดพรีเมียมให้เลือกเพียบเลยค่ะ ตัวขายดีจะแนะนำเป็น S5 Premium Dark คั่วเข้มขมช็อกโกแลต หรือจะเป็นเมล็ดอาราบิก้า Colombia Peach Candy กลิ่นพีชชัด ๆ ค่ะ แอดไลน์ขอตารางราคาส่งยกลังที่ @decemberdaycoffee หรือทิ้งเบอร์ไว้ให้ติดต่อกลับได้เลยนะคะ"
  ];

  const generalPhrases = [
    "รับทราบข้อมูลและพร้อมประสานงานให้ทันทีเลยค่ะ สะดวกแอดไลน์ @decemberdaycoffee หรือติดต่อตรงคุณขวัญ 096-163-1456 หรือจะพิมพ์เบอร์โทรกลับทิ้งไว้เพื่อให้น้องธันวาโทรนัดหมายให้ดีคะ?",
    "เข้าใจแล้วค่ะ คุณลูกค้าสะดวกแอดไลน์ OA ของแบรนด์ที่ @decemberdaycoffee หรือติดต่อโดยตรงคุณขวัญ หรือจะทิ้งเบอร์ติดต่อไว้ให้หนูโทรประสานงานด่วนดีคะ?"
  ];

  const getRandomPhrase = (arr: string[]) => arr[Math.floor(Math.random() * arr.length)];

  let replyText = getRandomPhrase(helloPhrases);
  let transferTriggered = false;
  let transferDepartment = "none";
  
  if (wantsStaff) {
    replyText = getRandomPhrase(wantsStaffPhrases);
    transferTriggered = true;
    transferDepartment = "sales";
  } else if (isIssue) {
    if (hasPhone) {
      replyText = getRandomPhrase(isIssuePhonePhrases);
      transferTriggered = true;
      transferDepartment = "technician";
    } else {
      replyText = getRandomPhrase(isIssueNoPhonePhrases);
    }
  } else if (isCoffeeQuery) {
    if (hasPhone) {
      replyText = getRandomPhrase(hasPhoneCoffeePhrases);
      transferTriggered = true;
      transferDepartment = "sales";
    } else {
      if (matched.length > 0) {
